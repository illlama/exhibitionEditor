...
import { Artwork, EditorElementStyle, EditorElementType } from "types"
...
const EditorElement = ({
  ...
  const getImgStyle = async () => {
    ...
    await tmpImg.decode()
    let imageHeight =
      tmpImg.width > imgWidthThreshold
        ? (tmpImg.height * imgWidthThreshold) / tmpImg.width
        : tmpImg.height
    let imageWidth =
      tmpImg.width > imgWidthThreshold ? imgWidthThreshold : tmpImg.width
    return {
      width: `${
        style.width === "auto" ? imageWidth || currentStyle.width : style.width
      }px`,
      height: `${
        style.height === "auto"
          ? imageHeight || currentStyle.height
          : style.height
      }px`,
    }
  }
    ...
    <>
      {tagName === "RECTANGULAR" ? (
        ...
        <div
          ...
          onClick={() => keyToCurrentElements([elementRef.current])}
          onMouseDown={(e) =>
            isSelected && !isDoubleClicked && onDraggable(e, element)
          }
          ref={elementRef as RefObject<HTMLDivElement>}
          onKeyDown={(e) => e.stopPropagation()}
          onDoubleClick={(e) => {
            const editerbleDiv = elementRef.current!
